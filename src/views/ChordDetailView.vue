<template>
  <div :key="routeKey">
    ...
    <div class="controls mt-3">
      <div class="play-buttons">
        <v-btn
          ...
          class="play-button"
        >
          <v-icon icon="mdi-music-note-multiple" size="x-large" class="mr-1"></v-icon>
          Play Chord
        </v-btn>
        <v-btn
          size="large"
          ...
          class="play-button"
        >
          <v-icon icon="mdi-music-note" size="x-large" class="mr-1"></v-icon>
          Play Arpeggio
        </v-btn>
      </div>
    </div>
    ...
    <ChordKeyList v-if="chordData" :base-key="key" :current-chord-id="chordId" />
          ...
          <v-col v-for="(inversion, index) in inversions" :key="index">
            <div class="music-box">
              <div class="music-box-header">
                <InversionHeader
                  :inversion="inversion"
                  :title="`${index + 1}${getOrdinalSuffix(index + 1)} inversion`"
                />
              </div>
...
const playInversionChord = (index: number) => {
  if (inversionPianos.value[index] && inversions.value[index]?.notes) {
    // Play immediately without blocking or setting isPlaying
    ;(inversionPianos.value[index] as any)
      .playChordWithNotes(inversions.value[index].notes)
      .catch(() => {})
  }
}
